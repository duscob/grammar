...
int main(int argc, char **argv) {
  ...
  if (!sdsl::cache_file_exists(KEY_GRM_SAMPLE_VALUES, config)
      || !sdsl::cache_file_exists(KEY_GRM_SAMPLE_ROOTS_POSITIONS, config)
      || !sdsl::cache_file_exists(KEY_GRM_SAMPLE_POSITIONS, config)) {
    grammar::SLP<> slp;
    ...
    auto report_sample =
        [&tmp_sample_pos, &tmp_samples, &tmp_sample_roots_pos](auto _seq_pos, auto _sum, auto _root_pos) {
    ...
    sdsl::store_to_cache(sample_roots_pos,
                         KEY_GRM_SAMPLE_ROOTS_POSITIONS /*+ ("_" + std::to_string(FLAGS_max_size))*/,
                         config);
